...
(TODO) NOTE: while the data preprocessing and generation functionality are dependent on MIDI files and the event vocabulary in [Oore et al. 2018](https://arxiv.org/pdf/1808.03715.pdf), users should be able to use the training script to train their own Relative Attention Transformer on any dataset, provided the correct hyperparameters, and provided they have adequately preprocessed their data into a single PyTorch tensor. Do create an issue if something does not work as expected.
